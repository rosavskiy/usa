...
export async function generateCarbonReport(
  ...
  // Disclaimer text
  currentY += 15; // Add some spacing before disclaimer

  const disclaimerText = `This Greenhouse Gas Emissions Inventory report has been generated by Carbon Tracker using automated AI-based OCR technology and emission factors provided by the EPA (eGRID 2023). While every effort is made to ensure the accuracy of data extraction and calculations, the use of Artificial Intelligence involves a margin of error. This report is intended for informational and internal tracking purposes only and does not constitute a certified third-party audit. The reporting entity (${companyName}) maintains final responsibility for the accuracy of all data and compliance with regulatory requirements. Carbon Tracker disclaims all liability for any errors or omissions in the generated report.`;

  doc
    .fontSize(7)
    .fillColor("#555")
    .font("Helvetica")
    .text(disclaimerText, tableX, currentY, {
      width: tableWidth,
      align: "justify",
    });
