...
export async function generateRecommendations(userId: number): Promise<Recommendation[]> {
  ...
  try {
    const aiRecommendations = await generateAIRecommendations(categoryTotals, totalEmissions);
...
Emissions breakdown:
${Object.entries(categoryTotals).map(([cat, kg]) => `- ${cat}: ${kg.toFixed(1)} kg CO2e`).join('\n')}
 ...
 */
function getFallbackRecommendations(categoryTotals: Record<string, number>): Recommendation[] {
  ...
  if (categoryTotals.electricity > 100) {
    ...
    recommendations.push({
      ...
      potentialReduction: `Save ${Math.round(categoryTotals.electricity * 0.12 * 12)} kg CO2e and $${Math.round(categoryTotals.electricity * 0.12 * 12 * 0.35)}/year`,
      category: 'electricity',
      priority: 'high',
    });
  }
  ...
  if (categoryTotals.fuel > 50) {
    ...
    const savingsDollars = Math.round((categoryTotals.fuel / 8.89) * 0.15 * 12 * 3.50); // gallons * price

    recommendations.push({
      title: 'Optimize Fleet Routes',
      description: 'Use route optimization software to reduce fuel consumption by 10-20%',
      potentialReduction: `Save ${savingsKg} kg CO2e and $${savingsDollars}/year`,
      category: 'fuel',
      priority: 'high',
    });
  ...
  if (categoryTotals.gas > 50) {
    const savingsKg = Math.round(categoryTotals.gas * 0.20 * 12);
    const savingsDollars = Math.round(savingsKg * 0.25); // rough estimate

    recommendations.push({
      title: 'Improve Building Insulation',
      description: 'Upgrade insulation to reduce heating requirements by 15-25%',
      potentialReduction: `Save ${savingsKg} kg CO2e and $${savingsDollars}/year`,
      category: 'gas',
      priority: 'medium',
    });
  ...
  recommendations.push({
    ...
    description: 'Hire a certified auditor to identify customized reduction opportunities',
  ...
  return [
    {
      ...
      description: 'Start tracking emissions by uploading utility bills, fuel receipts, or energy statements',
    ...
    {
      title: 'Install Smart Thermostats',
      description: 'Programmable thermostats optimize HVAC schedules automatically',
