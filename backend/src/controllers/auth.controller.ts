...
const registerSchema = z.object({
  ...
  companyName: z.string().min(2),  state: z.string().length(2).optional(),});
...
export const login = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
  ...
  // Verify password
  const isValidPassword = await UserModel.verifyPassword(validated.password, user.password_hash);
  if (!isValidPassword) {
    throw new AppError('Invalid credentials', 401);
  }
...
export const resetPassword = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
  ...
  const success = await PasswordResetService.resetPasswordWithToken(token, newPassword);
